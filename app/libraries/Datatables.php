...
class Datatables {
    ...
    ($key_condition, $val = NULL, $backtick_protect = TRUE) {
    ...
    protected function get_ordering() {
        ...
        for ($i = 0; $i < intval($this->ci->input->get('iSortingCols')); $i++):
            if (isset($mColArray[intval($this->ci->input->get('iSortCol_' . $i))]) && in_array($mColArray[intval($this->ci->input->get('iSortCol_' . $i))], $columns) && $this->ci->input->get('bSortable_' . intval($this->ci->input->get('iSortCol_' . $i))) == 'true') {
                $column_with_space = $mColArray[intval($this->ci->input->get('iSortCol_' . $i))];
                $column_array = explode(' ', $column_with_space);
                $column = $column_array[0];
                $this->ci->db->order_by($column, $this->ci->input->get('sSortDir_' . $i));
            }
        endfor;
    }
    ...
    protected function get_filtering() {
        ...
        $sWhere = '';
        $sSearch = $this->ci->db->escape($this->ci->input->get('sSearch'));

        $sSearch = substr($sSearch, 1, -1);
        $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
        $columns = array_values(array_diff($this->columns, $this->unset_columns));
...
if (!function_exists('format_date')) {
    ...
    function format_date($tdate) {
        if ($tdate == "")
            return $tdate;
        return date('m/d/Y', strtotime($tdate));
    }
